...
def main():
    ...
    parser.add_argument('--start', type=int, default=0, help='start pdb index')
    parser.add_argument('--end', type=int, default=99, help='end pdb index')
    # if len(sys.argv[1:]) == 0:
    #     parser.print_help()
    #     exit()
    ...
    result_path = config.model.joint_ckpt[:-3] + '_' + str(config.sample.num_samples)
    ...
    # f = open("test_idx_name.txt", "w")
    # for i in range(0, 100):
    #     data = test_set[i]
    #     print(data.protein_filename)
    #     f.writelines(str(i) + "," + data.protein_filename + "\n")
        
    # quit()
    # luhao add
    
    logger.info(f'**************Start sampling... from PDB ID {args.start} to {args.end}**************')
    for i in range(args.start, args.end+1):
    # my_list = [20, 21, 22, 43, 44, 45,66, 67, 68]
    # for i in my_list:
        data = test_set[i]
        print("Now, data_id is:{}, protein path is: {} ".format(i, data.protein_filename))
