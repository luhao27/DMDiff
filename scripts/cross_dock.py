...
if __name__ == '__main__':
    ...
    parser.add_argument('--pro_idx', type=int, default=1)
    parser.add_argument('--mol_idx', type=int, default=1) 
    # parser.add_argument('--mol_idx', type=int, default=0, choices=[72,73])  
    ...
    # sdf_path = './sdf_%d/*'%(args.mol_idx)
    sdf_path = "./res_2.sdf"
    print(sdf_path)
    mols4crossdock = glob.glob(sdf_path)
    ...
    for i, data in enumerate(tqdm(mols4crossdock)):
        ...
        try:
            ...
            print(docking_results[0]['pose'])
            save_file = "1.pdb"
            # 将docking结果的pose写入pdbqt文件
            with open(save_file, 'w') as f:
                f.write(docking_results[0]['pose'])
