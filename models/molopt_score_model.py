...
class ScorePosNet3D(nn.Module):
    ...
    def get_diffusion_loss(
        ...
        # mark
        real_box = torch.randn(4, 3).to(ligand_pos)
        unique_masks = torch.unique(batch_ligand)
        for i in unique_masks:
            min_coords_real = torch.min(ligand_pos[i==batch_ligand], dim=0)[0]  # [3] 大小的张量，分别表示 x, y, z 的最小值
            max_coords_real = torch.max(ligand_pos[i==batch_ligand], dim=0)[0]  # [3] 大小的张量，分别表示 x, y, z 的最小值
            real_box_size =  max_coords_real -  min_coords_real
            real_box[i] = real_box_size
        
        pred_box = torch.randn(4, 3).to(pred_ligand_pos)
        unique_masks = torch.unique(batch_ligand)
        for i in unique_masks:
            min_coords_pred = torch.min(pred_ligand_pos[i==batch_ligand], dim=0)[0]  # [3] 大小的张量，分别表示 x, y, z 的最小值
            max_coords_pred = torch.max(pred_ligand_pos[i==batch_ligand], dim=0)[0]  # [3] 大小的张量，分别表示 x, y, z 的最小值
            pred_box_size =  max_coords_pred -  min_coords_pred
            pred_box[i] = pred_box_size
        
        # mark
        loss_box_fun = nn.MSELoss()
        loss_box = 0.001 * loss_box_fun(real_box, pred_box)
        
        if self.use_classifier_guide:
            loss = loss_pos + loss_v * self.loss_v_weight + loss_exp * self.loss_exp_weight + loss_box
        else:
