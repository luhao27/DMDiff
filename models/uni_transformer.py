...
class BaseX2HAttLayer(nn.Module):
    ...
    # luhao add
    def forward(self, h, r_feat, edge_feat, edge_index, e_w=None, dist=None):
        N = h.size(0)
        src, dst = edge_index
        hi, hj = h[dst], h[src]
        ...
        if USE_MIXED:
            ...
            q_global, q_local = q[:, :self.n_heads//2, :], q[:, self.n_heads//2:, :]
            k_global, k_local = k[:, :self.n_heads//2, :], k[:, self.n_heads//2:, :]
            v_global, v_local = v[:, :self.n_heads//2, :], v[:, self.n_heads//2:, :]
            
            # global attention
            alpha_global = scatter_softmax((q_global[dst] * k_global / np.sqrt(k.shape[-1])).sum(-1), dst, dim=0,
                                dim_size=N)  # [num_edges, n_heads]
            # print(alpha_global.shape)
            # perform attention-weighted message-passing
            m_global = alpha_global.unsqueeze(-1) * v_global  # (E, heads, H_per_head)
            # print(m_global.shape)
            output_global = scatter_sum(m_global, dst, dim=0, dim_size=N)  # (N, heads, H_per_head)
            # print(output_global.shape)
            output_global = output_global.view(-1, self.output_dim//2)
            ...
            # with torch.no_grad():
                # src, dst = edge_index
                # # 计算节点对之间的欧几里得距离
                # distance = torch.norm(r_feat[dst] - r_feat[src], dim=-1, keepdim=True)  # [num_edges, 1]
            # print(dist.max(), dist.min())
            # print(dist[0:40])
            

            # 定义距离阈值，距离小于该阈值时注意力更大
            threshold = 10  
            # 距离因子，越接近0距离则越接近1，否则递减    
            distance_factor = torch.exp(-dist / threshold)
                # print(dist[0:40])
                # print(distance_factor[0:40])
            # for i in range(0, 13):
            #    print(i, torch.exp(-torch.tensor(i) / threshold))
            # print(distance_factor[0:40])
            # quit()
                # quit()
            # 计算注意力权重
            attention_scores_local = (q_local[dst] * k_local / np.sqrt(k.shape[-1])).sum(-1)  # [num_edges, n_heads]
            # 基于距离因子调整注意力权重
            adjusted_attention_scores_local = attention_scores_local * distance_factor  # [num_edges, n_heads]

            # 使用调整后的注意力权重计算 softmax
            alpha_local = scatter_softmax(adjusted_attention_scores_local, dst, dim=0, dim_size=N)  # [num_edges, n_heads]

            # perform attention-weighted message-passing
            m_local = alpha_local.unsqueeze(-1) * v_local  # (E, heads, H_per_head)
            output_local = scatter_sum(m_local, dst, dim=0, dim_size=N)  # (N, heads, H_per_head)
            output_local = output_local.view(-1, self.output_dim//2)
            
            output = torch.cat([output_global, output_local], -1)
...
class BaseH2XAttLayer(nn.Module):
    ...
    # mark
    def forward(self, h, rel_x, r_feat, edge_feat, edge_index, e_w=None, dist=None):
        N = h.size(0)
        src, dst = edge_index
        hi, hj = h[dst], h[src]
        ...
        if USE_MIXED:
            ...
            # global attention
            alpha_global = scatter_softmax((q_global[dst] * k_global / np.sqrt(k.shape[-1])).sum(-1), dst, dim=0, dim_size=N)  # (E, heads)
            # Perform attention-weighted message-passing
            m_global = alpha_global.unsqueeze(-1) * v_global  # (E, heads, 3)
            output_global = scatter_sum(m_global, dst, dim=0, dim_size=N)  # (N, heads, 3)
            ...
            # 使用调整后的注意力权重计算 softmax
            alpha_local = scatter_softmax(adjusted_attention_scores_local, dst, dim=0, dim_size=N)  # [num_edges, n_heads]
            # alpha_local = scatter_softmax((q_local[dst] * k_local / np.sqrt(k.shape[-1])).sum(-1), dst, dim=0, dim_size=N)  # (E, heads)
            # Perform attention-weighted message-passing
            m_local = alpha_local.unsqueeze(-1) * v_local  # (E, heads//2, 3)
            output_local = scatter_sum(m_local, dst, dim=0, dim_size=N)  # (N, heads//2, 3)


            
            output = torch.cat([output_global, output_local], -2)
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, e_w=None, fix_x=False):
        ...
        for i in range(self.num_x2h):
            ...
            # mark
            h_out = self.x2h_layers[i](h_in, dist_feat, edge_feat, edge_index, e_w=e_w, dist=dist)
        ...
        for i in range(self.num_h2x):
            ...
            # mark
            delta_x = self.h2x_layers[i](new_h, rel_x, dist_feat, edge_feat, edge_index, e_w=e_w, dist=dist)
